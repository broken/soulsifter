...
class SongList extends BpmMixin(GenresMixin(PlaylistMixin(QueryMixin(SearchMixin(SearchOptionsMixin(SettingsMixin(SongMixin(SongTrailMixin(LitElement))))))))) {
  render() {
    let songListItems = this.songs.map(s => html`<song-list-item .song="${s}" mvmode="${this.searchOptions.mvRestrict}"></song-list-item>`);
    return html`
      <div @song-selected="${this.songSelected}">
        ${songListItems}
      </div>
    `;
  }
  ...
  search() {
    ...
    p.keys = this.searchOptions.keyRestrict && !!this.song ? this.song.tonicKeys : [];
    ...
    this.songs = ss.SearchUtil.searchSongs(p.q, p.bpm, p.keys, this.genres, omitSongs, this.settings.getInt('songList.limit'), this.searchOptions.mvRestrict, this.searchOptions.orderBy);
  }
