...
class SongList extends BpmMixin(GenresMixin(PlaylistMixin(QueryMixin(SearchMixin(SearchOptionsMixin(SettingsMixin(SongMixin(SongTrailMixin(LitElement))))))))) {
  render() {
    let songListItems = this.songs.map(s => html`<song-list-item .song="${s}" mvmode="${this.searchOptions.mvRestrict}" dynamicgenremode="${this.searchOptions.dynamicGenress}"></song-list-item>`);
    return html`
      ${songListItems}
    `;
  }
