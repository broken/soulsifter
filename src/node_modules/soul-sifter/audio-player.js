...
class AudioPlayer extends SettingsMixin(LitElement) {
  ...
  playAction(e) {
    if (this.shadowRoot.getElementById('playPause').icon === 'play_arrow') {
      this.possiblyCreateAndPlay();
    } else {
      this.pause();
    }
  }

  possiblyCreateAndPlay() {
    if (!!this.song && this.song.id != this.prevSongId) {
      this.destroyAudio();
      this.prevSongId = this.song.id;
      ...
      this.fs.access(filepath, this.fs.F_OK | this.fs.R_OK, (err) => {
        if (!err) {
          ...
          if (!this.song.googleSongId) {
            ...
            window.gpm.getStreamUrl(this.song.googleSongId, (err, url) => {
              if (err) {
