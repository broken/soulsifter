...
class YoutubeClient extends AlertsMixin(Object) {  // TODO show errors
  ...
  _getNewToken() {
    ...
    this.addAlert('Authorize this app by visiting this url: ', authUrl);
  }

  _isTokenExpired() {
    return this.oauth2Client.credentials.expiry_date < Date.now() + 12000;
  }
  ...
  _checkTokenExpiration() {
    if (this._isTokenExpired()) {
      ...
      fs.unlink(this.tokenPath, (err) => {
        if (err) {
          this.addAlert('Unable to remove token path.');
          console.error(err);
        }
      });
      this._getNewToken();
