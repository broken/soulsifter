const YoutubeClient = class {  // TODO show errors
  ...
  _authorize() {
    ...
    fs.readFile(this.tokenPath, (err, token) => {
      if (err) {
        this._getNewToken(this.oauth2Client, callback);
      } else {
  ...
  async addPlaylistEntry(entryId) {
    ...
    try {
      ...
      console.error("Unable to update playlist entries.");
      console.error(err);
    }
  }
  ...
  async updatePlaylistEntries(playlistId) {
    try {
      ...
      for (song of songs) {
        if (!song.youtubeId) {
          let response = await this._findSong(song);
          song.youtubeId = response.items[0].id;
          song.update();
        }
  ...
  _getPlaylistItems(playlist, nextPageToken=undefined) {
    ...
    return new Promise((resolve, reject) => {
      this.service.playlistItems.list(, (err, response) => {
        if (err) reject();
