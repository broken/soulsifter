export const GooglePlayMusic = class {
  ...
  init() {
    ...
    let email = this.settings.getString('google.email');
    let appKey = this.settings.getString('google.appKey');
    if (!email || !appKey) {
      console.error('No email and/or appKey for Google Music.')
      return;
    }
    playmusic.init({email: email, password: appKey}, (err) => {
      if(err) {
  ...
  getStreamUrl(gmid, callback) {
    init();
    this.playmusic.getStreamUrl(gmid, callback);
  }
