...
class PlaylistEdit extends LitElement {
    ...
    return html`
      ...
      <abstract-action-page @cancel="${this.exit}" @accept="${this.save}">
        <div class="fields">
          ...
          <paper-checkbox ?checked="${this.gmusicSync}" id="gsync">sync with Google Music</paper-checkbox>
        </div>
        <div class="genres">
          <genre-list id="genreList" .genres="${this.playlist.styles}" single-select></genre-list>
        </div>
  ...
  save(e) {
    ...
    this.gmusicSync = this.shadowRoot.getElementById('gsync').checked;
    // Do I need to store in an intermediate array?
    let styleIds = this.playlist.styles.map(function(g) { return g.id; });
    ...
    if (this.gmusicSync && !this.playlist.gmusicId) {
      window.gpm.createPlaylist(this.playlist);
    }
