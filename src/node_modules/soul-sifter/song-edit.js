...
class SongEdit extends LitElement {
      ...
      <div class="relative">
        <abstract-action-page id="abstractActionPage" @cancel="${this.skip}" @accept="${this.save}">
          <div class="fields">
            ...
            <paper-input label="BPM" value="${this.editedSong.bpm}" floatingLabel id="bpm"></paper-input>
            <paper-input label="Key" value="${this.editedSong.tonicKey}" floatingLabel id="tonicKey"></paper-input>
            <div class="cols">
              <div class="rows">
                <paper-checkbox ?checked="${this.editedSong.lowQuality}" id="lowQuality">Low Quality</paper-checkbox>
                <paper-checkbox ?checked="${this.editedSong.album.mixed}" id="album_mixed">Mixed</paper-checkbox>
                <paper-checkbox ?checked="${this.editedSong.trashed}" ?disabled="${this.songIsTrashed}" id="trashed">Trash</paper-checkbox>
              </div>
  ...
  artistChanged(e) {
    ...
    if (!!artist && this.taggedSong.artist == this.taggedSong.album.artist) {
  ...
  titleChanged(e) {
    ...
    if (!!title && this.taggedSong.title == this.taggedSong.album.name) {
  ...
  titleCase(str) {
    return str.replace(/\w\S*/g, txt => {
      if (/^[ft|feat|rmx|flip|reflip|mix|remix]\.?$/.test(txt.toLowerCase())) return txt.toLowerCase();
      let numChars = 0;
      let numUppers = 0;
  ...
  forceEdits() {
    ...
    this.shadowRoot.getElementById('artist').value = this.song.artist;
    this.shadowRoot.getElementById('track').value = this.song.track;
    this.shadowRoot.getElementById('title').value = this.song.title;
    this.shadowRoot.getElementById('remixer').value = this.song.remixer;
    this.shadowRoot.getElementById('comments').value = this.song.comments;
    this.shadowRoot.getElementById('curator').value = this.song.curator;
    this.shadowRoot.getElementById('rating').value = this.song.rating;
    this.shadowRoot.getElementById('bpm').value = this.song.bpm;
    this.shadowRoot.getElementById('tonicKey').value = this.song.tonicKey;
    ...
    this.shadowRoot.getElementById('album_artist').value = this.song.album.artist;
    this.shadowRoot.getElementById('album_name').value = this.song.album.name;
    this.shadowRoot.getElementById('albumpart_pos').value = this.song.albumPart.pos;
    this.shadowRoot.getElementById('albumpart_name').value = this.song.albumPart.name;
    this.shadowRoot.getElementById('album_label').value = this.song.album.label;
    this.shadowRoot.getElementById('album_catalogId').value = this.song.album.catalogId;
