...
class SongEdit extends LitElement {
  render() {
    let basicGenreItems = this.basicGenreList.map(g => html`<options-menu-item @click="${this.basicGenreSelected}">${g.name}</options-menu-item>`);
    return html`
      <abstract-action-page id="abstractActionPage" @cancel="${this.skip}" @accept="${this.save}">
        ...
        <div class="fields">
          ...
          <div class="selectContainer">
            <paper-input label="Artist Genre" value="${this.basicGenreName}" required @focused-changed="${this.basicGenreFocusChanged}"></paper-input>
            <div id="basicGenreSelect" elevation="3">
              ${basicGenreItems}
            </div>
          </div>
  ...
  basicGenreFocusChanged(e) {
    if (e.detail.value) {
      this.shadowRoot.getElementById('basicGenreSelect').classList.add('show');
    }
  }
  basicGenreSelected(e) {
    window.console.log(e);
    this.shadowRoot.getElementById('basicGenreSelect').classList.remove('show');
    let i = 0;
    ...
    this.basicGenreName = e.path[i].innerText;
  }
/*
    observers: [
