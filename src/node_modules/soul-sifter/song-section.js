...
class SongSection extends SettingsMixin(SongMixin(SongTrailMixin(LitElement))) {
      ...
      <div class="content">
        <div class="row">
          ...
          <options-menu class="self-end">
            <options-menu-item @click="${this.openEditMixPage}">Add mix</options-menu-item>
        ...
        <div id="musicVideoInput" ?hide="${!!this.musicVideo}">
          <paper-input label="video id" no-label-float id="videoIdInput"></paper-input>
  ...
  songChanged(song) {
    ...
    this.clearCache = true;
    this.shadowRoot.getElementById('videoIdInput').value = '';
  ...
  associateVideo() {
    if (!this.song) return;
    let videoId = this.shadowRoot.getElementById('videoIdInput').value;
    this.setMusicVideo(ss.MusicVideoService.associateYouTubeVideo(this.song, videoId));
  }
  ...
  openEditMixPage(e) {
    ...
    var mix = new ss.Mix();
    mix.inSongId = this.songTrail[len - 2].songId;
    mix.outSongId = this.songTrail[len - 1].songId;
