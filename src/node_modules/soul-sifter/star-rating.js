import { } from "@material/mwc-icon";
import { css, html, LitElement } from "@polymer/lit-element";

class StarRating extends LitElement {
  render() {
    let stars = this.stars.map(s => this.isSelected(s) ? html`<mwc-icon @click=${this.select(s)} selected>star</mwc-icon>`
                                                       : html`<mwc-icon @click=${this.select(s)}>star_border</mwc-icon>`);
    return html`
      ${stars}
    `;
  }
